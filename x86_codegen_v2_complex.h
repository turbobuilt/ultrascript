...
public:
    ...
    // Debug mode support (required by base interface)
    bool is_debug_mode() const override { return debug_mode_enabled; }
    void set_debug_mode(bool enabled) override { debug_mode_enabled = enabled; }
    
    // Advanced JIT methods for pure x86 assembly generation
    void emit_byte(uint8_t byte) override;
    void emit_u32(uint32_t value) override;
    void emit_u8(uint8_t value) override;
    void emit_comment(const char* comment) override;
    void emit_string_literal(const char* str) override;
    void emit_call_external(const char* func_name) override;
    
    // Label and jump management for pure JIT code
...
private:
    ...
    std::vector<std::pair<int, size_t>> pending_label_patches; // label_id, offset
    int next_label_id = 1;
};
