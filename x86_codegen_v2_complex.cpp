...
// Helper function to benchmark code generation speed
...
// =============================================================================
// PURE JIT X86 ASSEMBLY METHODS FOR FREE KEYWORD
// =============================================================================
...
void X86CodeGenV2::emit_u8(uint8_t value) {
    code_buffer.push_back(value);
}

void X86CodeGenV2::emit_comment(const char* comment) {
    if (debug_mode_enabled) {
        // In debug mode, we could store comments for disassembly
        std::cout << "[ASM-COMMENT] " << comment << std::endl;
    }
    // Comments don't generate actual machine code
}

void X86CodeGenV2::emit_string_literal(const char* str) {
    // For debug messages, we need to embed the string in the code section
    // This is a simplified implementation - in practice we'd use a data section
    size_t len = strlen(str);
    for (size_t i = 0; i < len; ++i) {
        emit_byte(static_cast<uint8_t>(str[i]));
    }
    emit_byte(0); // null terminator
}

void X86CodeGenV2::emit_call_external(const char* func_name) {
    // Emit a call to an external function (like system free())
    // This generates a call instruction with a placeholder that will be resolved by the linker
    
    // For now, emit a direct call with placeholder address
    emit_byte(0xE8); // call rel32 instruction
    
    // Store the function name for later resolution
    unresolved_jumps.push_back({std::string(func_name), code_buffer.size()});
    
    // Emit placeholder 32-bit offset (will be patched later)
    emit_u32(0xDEADBEEF);
    
    if (debug_mode_enabled) {
        std::cout << "[JIT-DEBUG] External call to " << func_name << " at offset " << code_buffer.size() - 4 << std::endl;
    }
}
...
void X86CodeGenV2::mark_jump_location(int label_id) {
    // Store the location where a jump should be patched
    pending_label_patches.push_back({label_id, code_buffer.size()});
    
    if (debug_mode_enabled) {
        std::cout << "[JIT-DEBUG] Marked jump location for label " << label_id << " at offset " << code_buffer.size() << std::endl;
    }
}

void X86CodeGenV2::patch_jump(int label_id) {
    // Find and patch all jumps to this label
    ...
    for (auto it = pending_label_patches.begin(); it != pending_label_patches.end();) {
        if (it->first == label_id) {
            size_t jump_location = it->second;
            
            // Calculate relative offset (signed 8-bit for short jumps)
            int32_t offset = static_cast<int32_t>(current_pos - jump_location - 1);
            
            if (offset >= -128 && offset <= 127) {
                // Patch the 8-bit offset
                code_buffer[jump_location] = static_cast<uint8_t>(offset);
            } else {
                std::cerr << "[JIT-ERROR] Jump offset too large for 8-bit: " << offset << std::endl;
                // Could implement 32-bit jump patching here
            }
            
            if (debug_mode_enabled) {
                std::cout << "[JIT-DEBUG] Patched jump for label " << label_id 
                          << " at " << jump_location << " with offset " << offset << std::endl;
            }
...
void X86CodeGenV2::emit_jump_back(int label_id) {
    // Find the target location for this label
    size_t target_pos = 0;
    bool found = false;
    
    // In a real implementation, we'd store label positions
    // For now, assume we're jumping back to a previous location
    
    // Emit a backward jump instruction
    emit_byte(0xEB); // jmp short
    
    // Calculate offset (this is simplified - in practice we'd need to track label positions)
    int8_t offset = -10; // placeholder - would calculate actual offset
    emit_byte(static_cast<uint8_t>(offset));
    
    if (debug_mode_enabled) {
        std::cout << "[JIT-DEBUG] Emitted backward jump for label " << label_id << std::endl;
    }
}
