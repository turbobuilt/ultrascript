    ...
    if (current_scope) {
        for (auto* decl : current_scope->declared_functions) {
            if (decl && decl->name == function_var_name) {
                // TEMPORARY SIMPLIFIED: Skip complex function variable initialization
                // We're using direct calls, so we don't need function instances for now
                std::cout << "[FUNCTION_CODEGEN] SIMPLIFIED: Skipping function variable initialization for '" 
                          << function_var_name << "' (using direct calls)" << std::endl;
                break;
...
void generate_direct_function_call(CodeGenerator& gen,
    ...
    // TEMPORARY SIMPLIFIED APPROACH: Direct call by function name
    // This bypasses the complex function instance system for now
    // The function will be called directly by its generated label
    
    std::cout << "[FUNCTION_CODEGEN] Using simplified direct call to function '" 
              << function_var_name << "'" << std::endl;
    
    // Direct call to the function label
