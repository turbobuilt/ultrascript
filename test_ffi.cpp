...
int main() {
    ...
    void* lib = ultraScript::ffi_dlopen("./libtest_ffi.so");
    if (!lib) {
        std::cerr << "Failed to load library: " << ultraScript::ffi_last_error() << std::endl;
        return 1;
    }
    ...
    void* test_add_sym = ultraScript::ffi_dlsym(lib, "test_add");
    void* test_print_hello_sym = ultraScript::ffi_dlsym(lib, "test_print_hello");
    void* test_print_string_sym = ultraScript::ffi_dlsym(lib, "test_print_string");
    void* test_multiply_double_sym = ultraScript::ffi_dlsym(lib, "test_multiply_double");
    void* test_return_ptr_sym = ultraScript::ffi_dlsym(lib, "test_return_ptr");
    
    if (!test_add_sym || !test_print_hello_sym || !test_print_string_sym || 
        !test_multiply_double_sym || !test_return_ptr_sym) {
        std::cerr << "Failed to load symbols: " << ultraScript::ffi_last_error() << std::endl;
        return 1;
    }
    ...
    ultraScript::ffi_call_direct_void(test_print_hello_sym);
    std::cout << "✓ Direct void call successful" << std::endl;
    
    // Test 4: Direct calls - int64 return
    std::cout << "\n4. Testing direct int64 call..." << std::endl;
    int64_t result = ultraScript::ffi_call_direct_int64_i64_i64(test_add_sym, 42, 24);
    ...
    ultraScript::ffi_call_direct_void_ptr(test_print_string_sym, const_cast<char*>(test_str));
    std::cout << "✓ Direct string call successful" << std::endl;
    
    // Test 6: Direct calls - double
    std::cout << "\n6. Testing direct double call..." << std::endl;
    double double_result = ultraScript::ffi_call_direct_double_double_double(test_multiply_double_sym, 3.14, 2.0);
    ...
    void* ptr_result = ultraScript::ffi_call_direct_ptr_ptr(test_return_ptr_sym, test_ptr);
    ...
    ultraScript::ffi_clear_args();
    ultraScript::ffi_set_arg_int64(0, 100);
    ultraScript::ffi_set_arg_int64(1, 200);
    int64_t legacy_result = ultraScript::ffi_call_int64(test_add_sym);
    ...
    void* mem = ultraScript::ffi_malloc(1024);
    if (mem) {
        std::cout << "✓ ffi_malloc successful" << std::endl;
        ultraScript::ffi_memset(mem, 0x42, 100);
        std::cout << "✓ ffi_memset successful" << std::endl;
        
        void* mem2 = ultraScript::ffi_malloc(100);
        ultraScript::ffi_memcpy(mem2, mem, 100);
        if (ultraScript::ffi_memcmp(mem, mem2, 100) == 0) {
            std::cout << "✓ ffi_memcpy and ffi_memcmp successful" << std::endl;
        } else {
            std::cerr << "✗ ffi_memcpy or ffi_memcmp failed" << std::endl;
        }
        
        ultraScript::ffi_free(mem);
        ultraScript::ffi_free(mem2);
    ...
    if (ultraScript::ffi_dlclose(lib)) {
